...
class PositionMetricByDealCalculator(MetricCalculator):
    ...
    def calculate_row(cls, deal:pd.Series, prev:pd.Series) -> PositionMetricByDeal:
        ...
        metric.action = deal["Action"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["action"]
        metric.comment = deal["Comment"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["comment"]
        metric.commission = deal["Commission"] + prev["commission"]
        metric.deal_id = deal["Deal"]
        metric.digits = deal["Digits"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["digits"]
        ...
        metric.timestamp_open = deal["TimeUTC"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["timestamp_open"]
        metric.timestamp_open_server = (
            deal["Time"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["timestamp_open_server"]
        )
        metric.volume = deal["Volume"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["volume"]
        metric.volume_closed = deal["VolumeClosed"] + prev["volume_closed"]
        metric.volume_remaining = metric.volume - metric.volume_closed
        metric.volume_ext = deal["VolumeExt"] if deal["Entry"] == MT5Manager.MTDeal.EnDealEntry.ENTRY_IN else prev["volume_ext"]
